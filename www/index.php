<html>
	<head>
		...
		<meta http-equiv="refresh" content="60; url=http://192.168.210.100/index.php">
		<style>
			td { 
				padding: 10px;
				text-align: center; 
	...
	<body bgcolor="#000000" text="#CACACA" style="font-size: 50px">
		...
		<?php 
			...
			elseif ($_POST['srch']) { 
				echo '<center>';
				system('/home/thepub/bin/search ' . $_POST['query']);
				echo '</center><br>';
			}
			elseif ($_POST['play']) { exec('/home/thepub/bin/play ' . $_POST['song'] . ' > /dev/null &'); }
			elseif ($_POST['volume']) { exec('/home/thepub/bin/volume ' . $_POST['volume']);  }
			elseif ($_POST['stop']) { exec('/home/thepub/bin/stop'); }
			elseif ($_GET['force']) { exec('/home/thepub/bin/play ' . $_GET['what'] . ' > /dev/null &'); }
		?>
		<form method="POST" action="http://192.168.210.100/">
